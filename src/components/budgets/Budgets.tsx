...
const BudgetItem = ({ 
  ...
  return (
    <div className={`group relative flex flex-col gap-4 rounded-2xl border ${isExceeded ? 'border-rose-500/40 bg-rose-500/10' : isHighVelocity ? 'border-amber-500/20 bg-amber-500/5' : 'border-white/5 bg-white/5'} p-5 transition-all hover:bg-white/10`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {isExceeded && <p className="mt-1 text-[10px] font-medium text-rose-400 uppercase tracking-wider">Budget Exceeded</p>}
            {!isExceeded && warning && <p className="mt-1 text-[10px] font-medium text-amber-400 uppercase tracking-wider">High Velocity</p>}
          </div>
        </div>
        <div className="text-right">
          <p className="text-sm font-semibold text-white">
            ${spent.toLocaleString(undefined, { maximumFractionDigits: 0 })} <span className="text-neutral-500 font-normal">/ ${limit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <span className={`text-[10px] font-bold ${isExceeded ? 'text-rose-400' : isHighVelocity ? 'text-amber-500' : 'text-emerald-500'}`}>
    ...
    <div className="w-full space-y-10 py-6">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-emerald-500">Monthly Allocation</p>
          <h1 className="text-4xl font-semibold tracking-tight text-white">{currentMonth} Budgets</h1>
          <p className="max-w-xl text-sm leading-relaxed text-neutral-400">
            Your aggregate spending is currently <span className={`font-medium ${velocityDiff > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
              {Math.abs(Math.round(velocityDiff))}% {velocityDiff > 0 ? 'above' : 'below'}
            </span> projected velocity for this period.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-3xl border border-white/10 bg-[#121212] p-8 shadow-2xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Total Discretionary Spend</p>
              <h2 className="mt-2 text-5xl font-bold text-white">${Math.floor(totalSpent).toLocaleString()}<span className="text-2xl text-neutral-500">.{(totalSpent % 1).toFixed(2).substring(2)}</span></h2>
            </div>
            <div className="rounded-xl border border-white/5 bg-white/5 px-4 py-2 text-right">
              <p className="text-[10px] font-bold text-emerald-500 uppercase">{Math.round(totalPercentage)}% <span className="text-neutral-500 text-[8px]">of ${(totalLimit/1000).toFixed(1)}k limit</span></p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-neutral-500">
              ...
              <span>${Math.max(0, totalLimit - totalSpent).toLocaleString(undefined, { maximumFractionDigits: 0 })} remaining</span>
            </div>
        ...
        {worstCategory ? (
          <div className="rounded-3xl border border-rose-500/20 bg-[#121212] p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-semibold text-white">{worstCategory.category} Velocity High</h3>
                <p className="text-sm text-neutral-400 leading-relaxed">
                  Your '{worstCategory.category}' category is currently projected to exceed its allocation based on recent spending. You've spent {Math.round((worstCategory.spent/worstCategory.limit)*100)}% of the limit in {(currentDay/daysInMonth*100).toFixed(0)}% of the month time.
                </p>
              </div>
          ...
          <div className="rounded-3xl border border-emerald-500/20 bg-[#121212] p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-neutral-400 leading-relaxed">
                  All category allocations are well within projected bounds. Keep up the good work managing your resources.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-neutral-400">Active Allocations</h2>
        </div>
        
        <div className="grid grid-cols-1 gap-4">
          {initialBudgets.length === 0 && (
            <p className="text-sm text-neutral-500 text-center py-8">No budgets set for this month. Create your first category!</p>
          )}
